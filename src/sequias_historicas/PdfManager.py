...
class PdfManager:
    ...
    def check_month_cleaned(self, df: pd.DataFrame,pdfs: List[PdfFileInfo])-> pd.DataFrame:
        ...
        self._iterate_over_pdfs_by_class(pdfs, df, 'other', extra_filter, action, force_reprocess=True,msg="Setting isClean" ) # G67
    ...
    def check_image_hash_tool(self,hash:str, paper:str, year:int, month:int, day:int, page:int, ed:str=None)-> bool:
        filename = f"{year:04d}{month:02d}{day:02d}_{page:04d}" 
        if ed:
            filename += f"_{ed}"
        filepath =f"./data/datasets/clean/{paper}/{year:04}/{month:02}/{day:02}/{filename}.pdf"
        ...
        # execute pdfimages tool
        cmd = f"pdfimages -all {filepath} {tmp_path}/img"
        tool = Popen(cmd,shell=True)
        tool.wait()
        # read the generated image files and compute their hashes wihth md5
        
        img_file = None
        if  os.path.exists(f"{tmp_path}/img-000.jpg"):
            img_file = f"{tmp_path}/img-000.jpg"
        elif os.path.exists(f"{tmp_path}/img-000.ccitt"):
            img_file = f"{tmp_path}/img-000.ccitt"

        else:
            print("No image file generated by pdfimages")
            print(cmd)
            print(tool.stderr)
            print(tool.stdout)            
            exit(1)
        ...
        with open(img_file, "rb") as f:
            data = f.read()
            hash_real = md5(data,usedforsecurity=False).hexdigest()
        return hash_real==hash
