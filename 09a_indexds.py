...
def get_pred_sequia(file_name: str, pred_ds: pd.DataFrame) -> str:
...
def append_test(real_ds: pd.DataFrame, dataset: str, test_name: str) -> pd.DataFrame:
    ...
    def _get_pred_sequia(file_name: str, pred_ds: pd.DataFrame, bar) -> str:
...
def main():
    ...
    real_ds = real_ds[["file_name"]+tipos]
    ...
    output_file = f"results/{dataset}/classify/{dataset}_indexed_ds.csv"
